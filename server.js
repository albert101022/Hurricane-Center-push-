...
async function checkSismos(){
  ...
  try{
    // Feed publico GeoJSON - no requiere parametros, funciona sin restricciones
    const feedUrl = 'https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/2.5_day.geojson';
    const data = await fetchWithFallback(feedUrl);

    if(!data || !data.features){
      console.log('Sin respuesta USGS (todos los metodos fallaron)');
      return;
    }

    // Filtrar por region PR/Caribe: lat 15-22, lon -72 a -60
    const quakes = data.features.filter(function(f){
      const coords = f.geometry && f.geometry.coordinates;
      if(!coords) return false;
      const lon = coords[0], lat = coords[1];
      return lat >= 15 && lat <= 22 && lon >= -72 && lon <= -60;
    });

    console.log('USGS feed OK - '+data.features.length+' global, '+quakes.length+' en PR/Caribe');
